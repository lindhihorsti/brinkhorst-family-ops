...
# Reale Build-Metadaten (Commit + UTC-Zeit) an die Images durchreichen,
...
BUILD_META := GIT_SHA=$$(git rev-parse --short HEAD) BUILD_DATE=$$(date -u +%Y-%m-%dT%H:%MZ)

# =====================================================================
# DEV / TEST  (lokal, http://127.0.0.1:8080, eigene DB, AUTO_MIGRATE=1)
# =====================================================================
up:
	cd infra && $(BUILD_META) $(DEV) up -d --build
...
# =====================================================================
macmini-up:
	cd infra && $(BUILD_META) $(PROD) up -d --build
