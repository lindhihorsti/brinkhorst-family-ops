...
export default function KuechePage() {
  return (
    <>
        ...
        <main style={{
          ...
          <div style={{ maxWidth: "var(--page-max-width)", margin: "0 auto", padding: "var(--page-top-padding) var(--page-x-padding) var(--page-bottom-padding)" }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 20, marginTop: 4 }}>
              <span style={{
                width: 104, height: 104, borderRadius: 28, background: "#e8673a22",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: "clamp(56px, 7vw, 64px)",
              }}>🍳</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <h1 style={{ fontSize: "var(--font-size-xl)", fontWeight: 700, margin: 0 }}>Küchen- & Wochenplan</h1>
              <Link href="/" style={{
                ...
                {TILES.map((tile) => (
                  <Link key={tile.href} href={tile.href} style={{ textDecoration: "none" }}>
                    <div className="nav-tile" style={{
                      ...
                      <span style={{ width: 44, height: 44, borderRadius: 14, background: "#e8673a22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>{tile.icon}</span>
                      <div>
                ...
                {TILES.map((tile) => (
                  <Link key={tile.href} href={tile.href} style={{ textDecoration: "none", color: "inherit" }}>
                    <article className="nav-tile" style={{
                      ...
                      boxShadow: "var(--shadow-md)", padding: 20, minHeight: "var(--hub-tile-min-height)", display: "grid", alignItems: "space-between",
                    }}>
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, position: "relative" }}>
                        <div style={{ width: 72, height: 72, borderRadius: 24, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, background: "color-mix(in srgb, #e8673a 18%, var(--bg))", border: "1px solid color-mix(in srgb, #e8673a 38%, transparent)" }}>{tile.icon}</div>
                        <div style={{ width: 38, height: 38, borderRadius: 999, display: "flex", alignItems: "center", justifyContent: "center", background: "color-mix(in srgb, #e8673a 14%, var(--bg))", color: "#e8673a", fontSize: 22, fontWeight: 800, flexShrink: 0 }}>→</div>
                      </div>
      ...
      <PremiumHubLayout
        icon="🍳"
        title="Küche & Wochenplan"
        subtitle="Rezepte planen, Woche strukturieren, Einkauf vorbereiten"
        gradientFrom="var(--kueche-from)"
        gradientTo="var(--kueche-to)"
        backHref="/"
        navCurrent="/kueche"
        tiles={TILES}
      />
